...
class Timer {
  public:
    ...
    Timer(const std::string& name, const int num_data)
        : _name(name), _starts(num_data), _data(num_data), _is_stopped(num_data, true) {}
    ...
    void stop(const int data_point) {
        ...
        std::chrono::duration<double, std::micro> fp_micro = now - _starts[data_point];
...
void outputResults(std::ostream& stream, const std::string& data_point_name, const std::vector<Scalar>& data_point_vals,
                   const Timer& timer) {
    ...
    for (std::size_t n = 0; n < timer._data.size(); ++n) {
        ...
        // Compute the minimum.
        double local_min = *std::min_element(timer._data[n].begin(), timer._data[n].end());

        // Compute the maximum.
        double local_max = *std::max_element(timer._data[n].begin(), timer._data[n].end());

        // Compute the average.
        double local_sum = std::accumulate(timer._data[n].begin(), timer._data[n].end(), 0.0);
        double average = local_sum / timer._data[n].size();

        // Output.
        stream << data_point_vals[n] << " " << local_min << " " << local_max << " " << average << "\n";
    }
}
...
void outputResults(std::ostream& stream, const std::string& data_point_name, const std::vector<Scalar>& data_point_vals,
    ...
    for (std::size_t n = 0; n < timer._data.size(); ++n) {
        ...
        // Output on rank 0.
        if (0 == comm_rank) {
            stream << comm_size << " " << data_point_vals[n] << " " << global_min << " " << global_max << " " << average
                   << "\n";
        }
    }
}
